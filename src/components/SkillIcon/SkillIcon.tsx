...
export default function SkillIcon({ iconOnly = true, icon, title, size, knowsAboutMeta }: Props) {
    if (!icon?.name) {
        return null;
    }
    ...
    const iconSVG = icon.name !== 'accessibility' && icon.svg ? (
        <SVG aria-hidden={!iconOnly} style={{height: size, width: size}} src={icon?.svg} width={size} height={size} title={title} />
    ): (
        <svg aria-hidden={!iconOnly} role="img" fill="currentColor" width={size} height={size} viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg">
